...
import com.project.genassist_ecommerce.databinding.ViewFarmersBinding
...
class SellerAdapter(var list: List<Entries>, var onclick: (Entries) -> Unit) :
    ...
    class ViewFarmers(val binding: ViewFarmersBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(entries: Entries) {
            binding.sellerName.text = spanned("<b>Shop Name:</b> ${entries.name}")
            binding.sellerLocation.text = spanned("<b>Location:</b> ${entries.location}")
            binding.ratingBar.rating =  entries.rating.toFloat()
        }
